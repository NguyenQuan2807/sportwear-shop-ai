...
const getItemImage = (item) => {
  const rawImage =
    ...
    (Array.isArray(item?.productImages)
      ? item.productImages[0]?.imageUrl || item.productImages[0]?.url
      : null) ||
...
const getStatusButtonLabel = (currentStatus, status) => {
  if (currentStatus === status) {
    return `${status} • hiện tại`;
  }
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.2fr_0.8fr]">
              ...
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Tóm tắt người nhận và địa chỉ giao hàng.
                  </p>
                </div>
                <span
                  className={statusPillClassName(toneByStatus(order.status))}
                >
                  {order.status}
                </span>
              </div>
            ...
            <div className="rounded-[28px] border border-slate-200/70 bg-white p-6 shadow-sm">
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Chỉ cho phép chuyển theo luồng hợp lệ: PENDING →
                  CONFIRMED/CANCELLED → SHIPPING → DELIVERED → COMPLETED.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {STATUS_OPTIONS.map((status) => {
                  ...
                  const isAllowed = canTransitionTo(order.status, status);
                  ...
                  return (
                    <button
                      key={status}
                      type="button"
                      onClick={() => isAllowed && onUpdateStatus(status)}
                      disabled={isDisabled}
                      title={
                        isCurrent
                          ? "Trạng thái hiện tại"
                    ...
                    >
                      {updating && isAllowed
                        ? "Đang cập nhật..."
                        : getStatusButtonLabel(order.status, status)}
                    </button>
                  );
                })}
              </div>
          ...
          <section className="rounded-[28px] border border-slate-200/70 bg-white p-6 shadow-sm">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Chi tiết từng sản phẩm trong đơn, kèm ảnh để admin kiểm tra
                  nhanh.
                </p>
              </div>
            ...
            <div className="space-y-4">
              {order.items?.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="grid gap-4 rounded-[24px] border border-slate-200 p-4 md:grid-cols-[112px_minmax(0,1fr)_180px] md:items-center"
                  >
...
const InfoCard = ({ label, value, accent = false }) => {
  return (
    <div className="rounded-[20px] border border-slate-200 bg-slate-50/80 p-4">
      ...
      <p
        className={`mt-2 text-sm leading-6 ${accent ? "font-semibold text-indigo-600" : "text-slate-700"}`}
      >
        {value || "-"}
      </p>
    </div>
