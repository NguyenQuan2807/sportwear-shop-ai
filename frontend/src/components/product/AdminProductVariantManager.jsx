...
const sortVariants = (items) => {
  return [...items].sort((a, b) => {
    ...
    const numberA = Number(sizeA);
    const numberB = Number(sizeB);
...
const AdminProductVariantManager = ({ product, onClose }) => {
  ...
  const totalStock = useMemo(
    () => filteredVariants.reduce((sum, item) => sum + Number(item.stockQuantity || 0), 0),
    [filteredVariants],
  );
  ...
  const inventoryCostValue = useMemo(
    () =>
      filteredVariants.reduce(
        (sum, item) => sum + Number(item.costPrice || 0) * Number(item.stockQuantity || 0),
        0,
      ),
    [filteredVariants],
  );

  const variantsMissingCost = useMemo(
    () => filteredVariants.filter((item) => Number(item.stockQuantity || 0) > 0 && !Number(item.costPrice || 0)).length,
    [filteredVariants],
  );
      ...
      <div className="mx-auto max-w-7xl rounded-[32px] border border-slate-200/70 bg-[#F5F7FB] shadow-2xl">
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
            ...
            <AdminMetricCard label="Giá vốn tồn kho" value={formatCurrency(inventoryCostValue)} helper={variantsMissingCost > 0 ? `${variantsMissingCost} biến thể thiếu giá nhập` : "Đã đủ dữ liệu giá nhập"} tone="amber" />
          </div>

          <AdminCard
            title="Bộ lọc biến thể"
            description="Tìm theo màu, size, SKU, tồn kho, giá bán hoặc giá nhập. Giá nhập sẽ được AI dùng để chặn khuyến mãi bán dưới vốn."
            action={<AdminButton type="button" variant="brand" onClick={handleCreateClick}>Thêm biến thể</AdminButton>}
          >
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Ví dụ: Đỏ, M, 42, SKU001..."
            ...
            <div className="space-y-6">
              {groupedVariants.map(([color, items]) => (
                <AdminCard
                  key={color}
                  title={color}
                  description={`${items.length} biến thể • Tổng tồn kho ${items.reduce((sum, item) => sum + Number(item.stockQuantity || 0), 0)}`}
                >
                  <AdminTableShell>
                      ...
                      <table className="min-w-[980px] text-sm">
                        <thead className="bg-slate-50 text-left text-slate-500">
                          <tr>
                            ...
                            <th className="px-5 py-4 font-semibold">Giá nhập</th>
                            <th className="px-5 py-4 font-semibold">Biên LN</th>
                        ...
                        <tbody className="divide-y divide-slate-200/80 bg-white">
                          {items.map((variant) => {
                            const marginPercent = calculateMarginPercent(variant);
                            ...
                            return (
                              <tr key={variant.id} className="hover:bg-slate-50/70">
                                ...
                                <td className="px-5 py-4 text-slate-700">
                                  {variant.costPrice || variant.costPrice === 0 ? formatCurrency(variant.costPrice) : "-"}
                                </td>
                                <td className="px-5 py-4">
                                  {marginPercent === null ? (
                                    <span className="rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-bold text-amber-700">Thiếu giá nhập</span>
                                  ) : (
                                    <span className={`rounded-full border px-2 py-1 text-xs font-bold ${marginPercent <= 10 ? "border-rose-200 bg-rose-50 text-rose-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
